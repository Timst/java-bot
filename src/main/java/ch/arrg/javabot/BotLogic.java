...
public class BotLogic {
	...
	protected void onMessage(Bot bot, String channel, String sender, String login, String hostname, String message) {
		...
		for (CommandHandler handler : handlers.values()) {
			try {
				handler.handle(bot, channel, sender, login, hostname, message);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private void onHelp(Replyer rep, String message) {

		String handlerName = HandlerUtils.getWord(message, 1);
		if (handlers.containsKey(handlerName)) {
			handlers.get(handlerName).help(rep, message);
		} else {

			rep.send("Here are known handlers: ");
			StringJoiner join = new StringJoiner(" ");
			for (CommandHandler handler : handlers.values()) {
				join.add(handler.getName());
			}
			rep.send(join.toString());
			rep.send("Send +help <handler> to get more info on a particular one.");
		}
	}
